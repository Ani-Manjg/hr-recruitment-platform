...
export default function CandidateManagement() {
  ...
  const [statusById, setStatusById] = useState<Record<number, Candidate['status']>>({})
  const pageSize = 6
  const allCandidates = [...candidates, ...addedCandidates].map(candidate => ({ ...candidate, status: statusById[candidate.id] ?? candidate.status }))
  const locations = [...new Set(allCandidates.map(candidate => candidate.location))]
  ...
  const visibleCandidates = allCandidates.filter(candidate => {
    ...
    return matchesTab && matchesSearch && (locationFilter === 'all' || candidate.location === locationFilter) && candidate.score >= minimumScore
  })
  ...
  const shortlistCandidate = (id: number) => setStatusById(current => ({ ...current, [id]: 'shortlisted' }))
  ...
  return <main className="p-8">
    ...
    <div className="mt-7 flex items-center justify-between">
      ...
      <div className="flex gap-3"><label className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4"><Search className="size-4 text-slate-400"/><input value={search} onChange={event => { setSearch(event.target.value); setPage(1) }} className="w-40 py-2.5 text-sm outline-none" placeholder="Search name or skill"/></label><select aria-label="Sort candidates" value={sortBy} onChange={event => { setSortBy(event.target.value as SortOption); setPage(1) }} className="rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none"><option value="score-desc">AI Match Score (High to Low)</option><option value="score-asc">AI Match Score (Low to High)</option><option value="name">Name (A-Z)</option><option value="experience">Experience (Most)</option></select><div className="flex rounded-xl border border-slate-200 bg-white p-1"><button aria-label="Grid view" onClick={() => setView('grid')} className={`rounded-lg p-2 ${view === 'grid' ? 'bg-slate-100 text-blue-600' : 'text-slate-400'}`}><Grid2X2 className="size-4"/></button><button aria-label="List view" onClick={() => setView('list')} className={`rounded-lg p-2 ${view === 'list' ? 'bg-slate-100 text-blue-600' : 'text-slate-400'}`}><List className="size-4"/></button></div><button aria-label="Open filters" onClick={() => setFilterOpen(current => !current)} className={`rounded-xl border p-3 ${filterOpen || locationFilter !== 'all' || minimumScore > 0 ? 'border-blue-200 bg-blue-50 text-blue-600' : 'border-slate-200 bg-white'}`}><SlidersHorizontal className="size-4"/></button></div>
    </div>
    {filterOpen && <section className="card mt-5 flex flex-wrap items-end gap-4 p-5"><label className="text-sm font-semibold text-slate-700">Location<select value={locationFilter} onChange={event => { setLocationFilter(event.target.value); setPage(1) }} className="mt-1 block rounded-xl border border-slate-200 bg-white px-3 py-2 font-normal outline-none"><option value="all">All locations</option>{locations.map(location => <option key={location} value={location}>{location}</option>)}</select></label><label className="text-sm font-semibold text-slate-700">Minimum AI match score<span className="ml-2 text-blue-600">{minimumScore}%</span><input aria-label="Minimum AI match score" type="range" min="0" max="100" step="5" value={minimumScore} onChange={event => { setMinimumScore(Number(event.target.value)); setPage(1) }} className="mt-2 block accent-blue-600"/></label><button onClick={() => { setLocationFilter('all'); setMinimumScore(0); setPage(1) }} className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold hover:bg-[#f7f8fb]">Reset filters</button></section>}
    <div className={`mt-7 gap-5 ${view === 'grid' ? 'grid md:grid-cols-2 xl:grid-cols-3' : 'space-y-3'}`}>{pageCandidates.map(c => <Link to={`/candidates/${c.id}`} key={c.id} className={`card relative block p-5 transition hover:-translate-y-0.5 hover:shadow-md ${view === 'list' ? 'md:flex md:items-center md:gap-8' : ''}`}><label onClick={event => event.stopPropagation()} className="absolute right-4 top-4 z-10 flex items-center gap-2 rounded-lg bg-white/90 px-2 py-1 text-xs font-semibold text-slate-600 shadow-sm"><input type="checkbox" checked={selectedIds.includes(c.id)} onChange={() => toggleCandidate(c.id)} disabled={!selectedIds.includes(c.id) && selectedIds.length === 3} className="accent-blue-600"/>Compare</label><div className="flex items-center gap-3"><div className="grid size-12 place-items-center rounded-full bg-gradient-to-br from-blue-100 to-violet-100 font-bold text-blue-700">{c.name.split(' ').map(n => n[0]).join('')}</div><div className="min-w-0"><h2 className="font-bold">{c.name}</h2><p className="truncate text-sm text-slate-500">{c.role}</p></div></div><div className={`mt-4 flex gap-4 text-xs text-slate-500 ${view === 'list' ? 'md:mt-0' : ''}`}><span className="flex items-center gap-1"><MapPin className="size-3"/>{c.location}</span><span className="flex items-center gap-1"><Clock3 className="size-3"/>{c.experience}</span></div><div className={`mt-5 rounded-xl bg-blue-50 p-4 ${view === 'list' ? 'md:ml-auto md:mt-0 md:min-w-44' : ''}`}><div className="flex items-center justify-between text-sm font-semibold text-blue-700"><span>AI MATCH SCORE</span><span>{c.score}%</span></div></div><div className={view === 'list' ? 'md:ml-4 md:min-w-48' : ''}><p className="mb-2 mt-5 text-xs text-slate-500">Top Skills</p><div className="flex flex-wrap gap-1.5">{c.skills.map(s => <Badge key={s} className="bg-violet-50 text-violet-600">{s}</Badge>)}</div></div><button onClick={event => { event.preventDefault(); shortlistCandidate(c.id) }} disabled={c.status === 'shortlisted'} className={`mt-5 flex items-center justify-center gap-2 rounded-lg border py-2 text-sm font-semibold ${c.status === 'shortlisted' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-slate-200'} ${view === 'list' ? 'md:ml-4 md:w-32' : 'w-full'}`}><UserCheck className="size-4 text-emerald-500"/>{c.status === 'shortlisted' ? 'Shortlisted' : 'Shortlist'}</button></Link>)}</div>
