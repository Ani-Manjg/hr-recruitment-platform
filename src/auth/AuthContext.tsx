...
export function AuthProvider({children}:{children:ReactNode}){
  ...
  useEffect(()=>{localStorage.removeItem('talentflow-auth-token');const unsubscribe=subscribeApiSession(session=>{setAccessToken(session?.accessToken||null);setCsrfToken(session?.csrfToken||null);if(!session)setUser(null)});const savedCsrf=sessionStorage.getItem(CSRF_STORAGE_KEY);seedCsrfToken(savedCsrf);if(!savedCsrf){setIsLoading(false)}else{refreshSession().catch(clear).finally(()=>setIsLoading(false))}return unsubscribe},[clear,refreshSession])
